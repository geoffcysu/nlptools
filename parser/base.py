...
from typing import Any, Callable, FrozenSet, Generic, TypeVar, Optional, Sequence

# ------------------------------------------------------------------
# This module is modified from the parsy parser combinator package on PyPI.
# ------------------------------------------------------------------
...
_Tok,_A,_B = TypeVar('_Tok'),TypeVar('_A'),TypeVar('_B')
_Stream = TypeVar('_Stream')

@dataclass
class Result(Generic[_A]):
    status: bool
    index: int
    value: _A
    furthest: int
    ...
    @staticmethod
    def success(index, value:_A)->Result[_A]:
        return Result(True, index, value, -1, frozenset())

    @staticmethod
    def failure(index, expected)->Result[Any]:
        return Result(False, -1, None, index, frozenset([expected]))

    # collect the furthest failure from self and other
    def aggregate(self, other:Optional[Result[Any]]):
        if not other:
            return self
...
#supported stream type: str | bytes | list

class Parser(Generic[_Stream,_A]):
    """
    ...
    def __init__(self, wrapped_fn: Callable[[_Stream, int], Result[_A]]):
        """
    ...
    def __call__(self, stream: _Stream, index: int):
        return self.wrapped_fn(stream, index)

    def parse(self, stream: _Stream) -> _A:
        """Parses a string or list of tokens and returns the result or raise a ParseError."""
        (result, _) = (self << eof).parse_partial(stream)
        return result

    def parse_partial(self, stream: _Stream) -> tuple[_A, _Stream]:
        """
        ...
        """
        assert isinstance(stream,Sequence), "stream should be a Sequence"
    ...
    def bind(self, bind_fn:Callable[[_A],Parser[_Stream,_B]])->Parser[_Stream,_B]:
        @Parser
    ...
    def map(self, map_function: Callable[[_A],_B]) -> Parser[_Stream,_B]:
        """
    ...
    def combine(self, combine_fn: Callable[...,_B]) -> Parser[_Stream,_B]:
        """
    ...
    def combine_dict(self, combine_fn: Callable[...,_B]) -> Parser[_Stream,_B]:
        """
...
class SeqParser(Generic[_Stream,_A],Parser[_Stream,_A]):...
...
def success(value: _A) -> Parser[_Tok,_A]:
    """
